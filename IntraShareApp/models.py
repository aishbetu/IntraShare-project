...
class IntraResponse(models.Model):
...
class responseView(models.Model):
    response = models.ForeignKey(IntraResponse, on_delete=models.CASCADE)
    ip = models.CharField(max_length=200, null=True)
    session = models.CharField(max_length=200, null=True)
    created = models.DateTimeField(default=timezone.now)

class Comment(models.Model):
    response = models.ForeignKey(IntraResponse, on_delete=models.CASCADE, related_name='comments')
